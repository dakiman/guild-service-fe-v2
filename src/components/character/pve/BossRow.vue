<template>
  <div
    class="flex items-center gap-3 p-2 rounded wsa-card-inner"
  ...
  >
    <div class="shrink-0 w-10 h-10 rounded overflow-hidden bg-base-300 flex items-center justify-center">
      <img
        ...
        loading="lazy"
      />
      <Skull v-else class="w-5 h-5 text-wsa-muted/40" />
    </div>

    <div class="flex flex-col flex-1 min-w-0">
      <span class="text-sm text-wsa-text truncate">{{ encounter.name }}</span>
      <span v-if="progress" class="text-[11px] text-wsa-muted/60 truncate">
