...
`components/feedback/FreshnessChips.vue` renders 10 slice chips in fixed order: `Profile, M+, PvP, Profs, Raids, Stats, Titles, Reps, Collect., Achievs`. **Order must match `meta.freshness` keys from BE — drift is silent.**
