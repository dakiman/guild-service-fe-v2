...
The app is **Peon** (peon.pro). Wordmark is always typeset text "PEON", never a raster. `public/favicon.svg` is the hand-authored brand mark and single source — it doubles as the AppNav logo (`<img src="/favicon.svg">`) and gets rasterized into `apple-touch-icon.png`. Mascot art in `public/brand/*.jpg` (+ `og-image.jpg`, `apple-touch-icon.png`) is generated by `node scripts/process-brand-assets.mjs` from source art in `~/dev/guild-branding/round2-peon/` (outside the repo — regen requires that dir). Decorative brand `<img>`s take `alt=""` + `aria-hidden="true"` and must not break layout when missing. **Fonts** are self-hosted via `@fontsource` (latin woff2, imported in `main.ts`): body = Merriweather Sans variable (Tailwind `font-sans`), display = Cinzel (`font-display`, also baked into `.wsa-text-heading`/`.wsa-wordmark`). Display goes on the wordmark, page-level h1s and card headings only — never body copy or tables. Flavor quips (exact strings): loading `Work, work…` · generic error `Job's… not done.` · not-found `Nothing need doing here.` · sync complete `Job's done!`
