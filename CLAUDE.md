...
`components/feedback/FreshnessSummary.vue` collapses `meta.freshness` into one aggregate pill ("Data up to date" / "N sections refreshing soon" / "Updating N sections…") with a click-to-open breakdown panel that renders `FreshnessChips.vue` — 10 slice chips in fixed order: `Profile, M+, PvP, Profs, Raids, Stats, Titles, Reps, Collect., Achievs`. **Chip order must match `meta.freshness` keys from BE — drift is silent.** A `hiddenKeys` prop filters slices out of both the aggregate and the panel. (Hidden entirely for basic-tier characters.)
